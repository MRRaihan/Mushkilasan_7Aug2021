...
/ 2), 2) )) AS distance");
    ...
    public function get_wallet_history_info($token) {
        $wallet = $this->db->select('id,token,payment_detail,user_provider_id,type,current_wallet,currency_code,credit_wallet,debit_wallet,avail_wallet,total_amt,fee_amt,reason,created_at,withdraw_request_id')
        ->from('wallet_transaction_history')
        ->where('token', $token)
        ->order_by('id', 'DESC')
        ->get();
        return $wallet->result_array();
    }
